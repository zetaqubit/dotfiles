return {
	...
	opts = {},
	config = function()
		-- Bindings to launch diffview.
    utils = require('z.utils')
		...
		map("n", "<leader>gt",
			utils.with_git_root(function(opts) require("diffview").open({ cwd = opts.cwd }) end),
			opts("Open Git [D]iff (working [t]ree vs HEAD)")
		)
        ...
        git_root = utils.get_last_vcs_file_git_root()
    ...
    -- Remember cursor position when leaving and coming back to commit history.
    vim.api.nvim_create_autocmd({"CursorMoved"}, {
      pattern = "DiffviewFileHistoryPanel",
      callback = function()
        vim.b._diffview_history_cursor = vim.fn.line(".")
      end,
    })
    vim.api.nvim_create_autocmd("BufWinEnter", {
      pattern = "DiffviewFileHistoryPanel",
      callback = function()
        local pos = vim.b._diffview_history_cursor
        if pos then
